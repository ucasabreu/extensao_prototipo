...
const META_HORAS = 360; // Meta curricular de extensão
...
function renderizarKPIs() {
    ...
    const emAnalise = certificacoesData.filter(c => c.status === "Em análise").length;
    ...
    if (!lista.length) {
        container.innerHTML = `
            <div class="certificacoes-vazio">
                ...
                <p style="margin-top: 8px; font-size: 12px;">Complete atividades de extensão para obter certificados.</p>
            </div>
        `;
        return;
    }
            ...
            <div class="certificacao-card ${statusClass}" onclick="abrirModalCertificacao(${c.id})">
                ...
                <div class="certificacao-card-body">
                    <div class="certificacao-horas">
                        ${c.carga}h <small>de extensão</small>
                    </div>
...
window.abrirModalCertificacao = function (id) {
    ...
    statusBadge.textContent = c.status === "Aprovada" ? "Certificado Disponível" : "Aguardando Validação";
...
window.gerarHistorico = function () {
    mostrarToast("info", "Gerando histórico completo de extensão...");
    // Simula geração
    setTimeout(() => {
        mostrarToast("success", "Histórico gerado! Verifique seus downloads.");
    }, 1500);
};
